...
const LuxuryGallery = () => {
    ...
    const handleAddToCart = (cartId) => {
        axios.post(`${apiUrl}/cart/${cartId}`,{
        userId: userId,
        goodsId: itemList.goods.id,
        image_url: itemList.goods.image_url,
        price: itemList.goods.price
      })
    };
                            ...
                            {itemList && itemList.map((e) => {
                                return (
                                    <div key={e.id}>
                                        ...
                                        <button onClick={() => handleAddToCart(itemList.id)} className="flex items-center justify-center w-full px-2 py-2 mt-4 font-medium tracking-wide text-white capitalize transition-colors duration-200 transform bg-gray-800 rounded-md hover:bg-gray-700 focus:outline-none focus:bg-gray-700">
