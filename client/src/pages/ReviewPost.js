...
const ReviewPost = () => {
  ...
  <div id="viewerBox" className="lg:p-10 md:p-6 p-4 bg-white dark:bg-gray-900 rounded-md">
    ...
    <div className="mt-3 md:mt-4 lg:mt-0 flex flex-col lg:flex-row items-strech justify-center lg:space-x-8">
      ...
      <div className="lg:w-1/2 flex flex-col justify-center mt-7 md:mt-8 lg:mt-0 pb-8 lg:pb-0">
        <div className="mb-10">
            <span className="text-white mr-5 text-xl">
                카테고리
            </span>
            <select onChange={onCategoryHandler}>{options}</select>
        </div>
    ...
    <textarea onChange={onContentHandler} value={content} className="flex-1 w-full px-4 py-2 text-base text-gray-700 placeholder-gray-400 bg-white border border-gray-300 rounded-lg appearance-none focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent" id="content" placeholder="Enter your content" name="content" rows="5" cols="40">
        ...
        <div className="flex items-center flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-6 lg:space-x-8 mt-8 md:mt-16">
          <button onClick={onSubmitHandler} className=" rounded-lg w-full md:w-3/5 border border-gray-800 text-base font-medium leading-none uppercase py-6 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 bg-gray-800 text-white dark:bg-white dark:text-gray-900 dark:hover:bg-gray-200">리뷰 작성하기</button>
        </div>
